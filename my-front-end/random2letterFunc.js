async function random_two_letter_func_v2 () {
  ...
  try {
  const result = await fetchDoggy.getFetchLetters(x)//fetch(`${switchboard.USERS_URL}/${x}/letters`)
  ...
  const two_random_letters = await the_letters.forEach(element => {
    ...
    const result = await fetchDoggy.getFetchWords(x) //fetch(`${switchboard.USERS_URL}/${x}/words`)
    const data = await result.json()
